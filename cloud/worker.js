...
async function resolveAccess(env, raw) {
  ...
  if (!config.deepseekKey) return { ok: false, status: 500, msg: '服务端还没配置 API Key，请管理员运行 node gen-codes.js key sk-xxx' };
  return { ok: true, key: config.deepseekKey, code: k, entry, today, limit };
}

// ── 扣次数（sk- 直通不计；await 写入保证计数可靠）──
...
async function route(request, env, ctx, path) {
  ...
  if (method === 'GET' && path === '/api/voice-health') {
    return json({ ok: true, stt: false });
  }

  if (method === 'POST' && path === '/api/voice') {
    return json({ error: '语音识别需要作者的电脑开机（本地模式），云端版暂不支持' }, 501);
  }

  if (method === 'GET' && path === '/api/diagnose') {
  ...
  if (method === 'POST' && path === '/api/translate') {
    ...
    const access = await resolveAccess(env, body.key);
...
5. 只返回译文本身，不要任何额外内容或格式` },
    ...
    return json({ text: translated, det: d.model === 'deepseek-v4-flash' ? 'DeepSeek V4 Flash' : 'DeepSeek Chat', model: d.model });
  }
...
词典质量对标俄罗斯科学院出版的《Большой толковый словарь русского языка》。释义准确、例句地道、标注完整。` },
  ...
  if (method === 'POST' && path === '/api/word-analysis') {
    ...
    if (cached) {
      const access = await resolveAccess(env, body.key); // 命中也先验码，但不计次数
      if (!access.ok) return json({ error: access.msg }, access.status);
      return json(cached);
    }
    const access = await resolveAccess(env, body.key);
...
规则：恰好 ${count} 道题；ans 是正确选项下标（0-3），各题分布随机；每题四个选项只有一个正确；难度贴合 ${level} 级；题目围绕给定主题；解析用中文。` },
    ...
    if (qs.length) {
      return json({ questions: qs, _model: model === 'deepseek-v4-flash' ? 'DeepSeek V4 Flash' : 'DeepSeek Chat' });
    }
